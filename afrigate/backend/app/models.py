...
class PlatformSettings(Base):
    ...
    # --- FX rates (units of local currency per 1 USD) ---
    # Auto-synced daily from open.er-api.com (free, no key) by
    # utils/fx_rates.py. An admin can manually override any value from the
    # dashboard - doing so does NOT stop future auto-syncs unless
    # fx_auto_update_enabled is turned off, so a manual edit is a one-time
    # correction by default, not a permanent lock.
    usd_to_tzs = Column(Float, nullable=False, default=2650.0)
    usd_to_kes = Column(Float, nullable=False, default=129.0)
    usd_to_ngn = Column(Float, nullable=False, default=1530.0)
    usd_to_ghs = Column(Float, nullable=False, default=15.5)
    usd_to_ugx = Column(Float, nullable=False, default=3700.0)

    fx_auto_update_enabled = Column(Boolean, nullable=False, default=True)
    fx_last_synced_at = Column(DateTime, nullable=True)
    fx_source = Column(String(60), nullable=True)  # e.g. "auto:open.er-api.com" | "manual"
